...
from src.helpers import init_helper, bbox_helper, video_helper, vsumm_helper, maniqa_helper
...
def save_output(source, boundaries, save_path, prediction_summary, thumbnail_indexes, write_with_moviepy=True):
    if thumbnail_indexes != None:
        for i in range(0, len(thumbnail_indexes)):
            cap = cv2.VideoCapture(source)
            cap.set(cv2.CAP_PROP_POS_FRAMES, thumbnail_indexes[i])
            _, frame = cap.read()
            output_path = f"{os.path.dirname(save_path)}/thumbnail_{i}.png" 
